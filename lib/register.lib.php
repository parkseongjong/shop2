...
function exist_mb_hp($reg_mb_hp, $reg_mb_id)
{
    ...
    if($row['cnt'])
        return " 이미 사용 중인 휴대전화번호입니다. ".$reg_mb_hp;
    else
        return "";
}


function find_id_by_phone($phone_no)
{
    global $g5;

    if (!trim($phone_no)) return "";
    $keyword = hyphen_hp_number($phone_no);
    $sql = "SELECT mb_id FROM {$g5['member_table']} WHERE mb_hp = " . fn_sql_quote($keyword) . " AND mb_leave_date = '' AND (mb_intercept_date = '' OR (mb_intercept_date != '' AND mb_intercept_date > CURRENT_DATE()))";
