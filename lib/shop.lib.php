...
function get_it_stock_qty($it_id)
{
    ...
    $sql = " select it_stock_qty, it_2 from {$g5['g5_shop_item_table']} where it_id = '$it_id' ";
    $row = sql_fetch($sql);

    if($row['it_2']) return 999999;
...
function get_order_info($od_id, $in_order = false)
{
    global $g5;
...
function save_order_point($ct_status="완료")
{
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        if ($od_row['mb_id'] && $row['ct_point'] > 0) { // 회원이면서 포인트가 0보다 크다면
            ...
            /** @chagngelog 22.06.13 Lee, Namdu 지급할 포인트에서 대리점 포인트 차감 */
            if(empty($row['mb_agent_code']) !== true) {
                $agent_point = (int)$row['ct_agent_margin'] * $row['ct_qty'];
                $agent_point > 0 && ($po_point = $po_point - $agent_point); // 지급할 포인트에서 대리점 영업 수당 포인트 차감
            }

            $po_point > 0 && insert_point($od_row['mb_id'], $po_point, $po_content, "@delivery", $od_row['mb_id'], "{$od_row['od_id']},{$row['ct_id']}");
        }
...
function save_agent_margin($order_id)
{
    global $g5;
    ...
    $oid = fn_sql_quote($order_id);
    $status = implode(', ', fn_sql_quote(['준비','배송','완료', '취소', '반품', '품절']));

    $sql = "SELECT ct_id, od_id, ct_status, it_name, mb_agent_code, ct_point, ct_agent_margin, ct_qty FROM {$g5['g5_shop_cart_table']} TM WHERE od_id = {$oid} AND ct_status IN($status) AND mb_agent_code !=''";
    $rows = fn_sql_fetch_all($sql);
    if (!sql_query('START TRANSACTION', true)) return false;


    $row = reset($rows);
    $agent_code = $row['mb_agent_code'];

    foreach ($rows as $row) {
        $point = min($row['ct_point'], $row['ct_agent_margin']) * $row['ct_qty'];
        if (!$point) continue;

        $note = "\"{$row['it_name']}\" 상품 주문({$row['od_id']}) ";

        // 포인트 차감 - 주문 취소
        if ( in_array($row['ct_status'], ['취소', '반품', '품절']) ) {

            // 기존에 지급된 포인트가 없는 경우 차감하지 않음
            if ($point != existAgentPointHistory($agent_code, $row['ct_id'])) continue;

            $point *= -1;
            $note .= $row['ct_status'] . ' 차감';
        }
        // 포인트 차감
        else {
            $note .= '결제 지급';
        ...
        $result = addAgentPoint($agent_code, $point, $row['ct_id'], $note);
    ...
    /* 누적합계 계산
    $total = (int)fn_sql_row("SELECT SUM(ap_point) FORM {$g5['tb_agent_point']} WHERE ap_agent={$agent_idx}");
    sql_query("
    UPDATE
        {$g5['tb_agent_list']}
    SET
        al_total_margin='{$total}'
    WHERE
        al_idx='{$agent_idx}'
    ");
    */
    if (!sql_query('COMMIT', true)) return false;
    return true;
