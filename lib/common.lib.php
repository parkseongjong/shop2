...
function is_admin($mb_id)
{
    ...
    if ($config['cf_admin'] == $mb_id || in_array($mb_id, ['devper', 'soonchan', 'lilyosia'])){
        $is_authority = 'super';
    } else if(in_array($mb_id, ['oper1', 'oper2', 'oper3'])) {
        $is_authority = 'operator';
...
function insert_point($mb_id, $point, $content='', $rel_table='', $rel_id='', $rel_action='', $expire=0)
{
    ...
    if($config['cf_point_term'] > 0 && $expire !== -1) {
        if($expire > 0)
...
function get_point_sum($mb_id)
{
    ...
    if($config['cf_point_term'] > 0) {
        ...
        $sql = "
            ...
            WHERE 
                mb_id = '$mb_id'
                AND po_expire_date BETWEEN '1970-01-01' AND DATE_SUB('".G5_TIME_YMD."', INTERVAL 1 DAY)
                AND po_expired != '1'
              ";
        sql_query($sql);
    }
    ...
    // 포인트합
    $sql = "SELECT sum(po_point) as sum_po_point FROM {$g5['point_table']} WHERE mb_id = '$mb_id' ";
    $row = sql_fetch($sql);
...
function get_expire_point($mb_id)
{
    ...
    $sql = "SELECT sum(po_point - po_use_point) as sum_point FROM {$g5['point_table']} ".
           "WHERE mb_id = '$mb_id' AND po_expired = '0' AND po_expire_date BETWEEN '1970-01-01' AND DATE_SUB('".G5_TIME_YMD."', INTERVAL 1 DAY)";
    $row = sql_fetch($sql);
...
function get_uniqid()
{
    ...
    do {
        ...
        $id = date('ymdhis') . substr(microtime(), 4, 4);
        if (sql_query("INSERT INTO {$g5['uniqid_table']} SET uq_id = '{$id}', uq_ip = '{$client_ip}'", false)) break; // 쿼리가 정상이면 빠진다.
    }
    while (true);
...
function certify_count_check($mb_id, $type)
{
    ...
    $limit = $config['cf_cert_limit'];
    $count = certify_get_count($mb_id, $type);
    $title = ['hp' => '휴대폰', 'ipin' => '아이핀'][$type];

    if($count >= $limit) alert_close("{$title} 본인확인은 일일 {$limit}회 까지만 사용 가능합니다.");
}


// 인증시도회수 체크
function certify_get_count($mb_id, $type)
{
    global $g5;
    $sql = "SELECT count(0) FROM {$g5['cert_history_table']} ";
...
function member_delete($mb_id)
{
    ...
    if ($mb['mb_recommend']) {
        !function_exists('fn_withdrawal_nominee') && include G5_EXTEND_PATH.'/helper.extend.php';

        fn_withdrawal_nominee($mb['mb_recommend'], $mb_id);
        //$row = sql_fetch(" select count(*) as cnt from {$g5['member_table']} where mb_id = '".addslashes($mb['mb_recommend'])."' ");
        //if ($row['cnt'])
        //insert_point($mb['mb_recommend'], $config['cf_recommend_point'] * (-1), $mb_id.'님의 회원자료 삭제로 인한 추천인 포인트 반환', "@recommend", $mb['mb_recommend'], $mb_id.' 추천인 삭제');
    }

    // 회원자료는 정보만 없앤 후 아이디는 보관하여 다른 사람이 사용하지 못하도록 함 : 061025
    $sql = " update {$g5['member_table']} set mb_password = '', mb_level = 1, mb_email = '', mb_homepage = '', mb_tel = '', mb_hp = '', mb_zip1 = '', mb_zip2 = '', mb_addr1 = '', mb_addr2 = '', mb_dupinfo = '', mb_birth = '', mb_sex = '', mb_certify = '', mb_signature = '', mb_memo = '".date('Ymd', G5_SERVER_TIME)." 삭제함\n".sql_real_escape_string($mb['mb_memo'])."' where mb_id = '{$mb_id}' ";
