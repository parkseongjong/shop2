...
else if ($od_settle_case != '무통장' && $od_settle_case != '가상계좌' && $od_settle_case != '신용카드' && $default['de_pg_service'] == 'payup') {
    if (function_exists('add_order_post_log')) add_order_post_log('유효하지 않은 결제 방식입니다.');
...
if ($od_settle_case == "무통장") {
    ...
    switch ($default['de_pg_service']) {
        ...
        case 'inicis':
            ...
            break;
        case 'payup' :
            include G5_SHOP_PATH . '/payup/payup_result.php';
            break;
        default:
            include G5_SHOP_PATH . '/kcp/pp_ax_hub.php';
    ...
    if ((int)$order_price !== (int)$pg_price) {
        ...
        switch ($od_pg) {
            ...
            case 'KAKAOPAY':
                ...
                break;
            case 'payup' :
                $_POST['transactionId'] = $tno;
                $_POST['reason'] = $cancel_msg;
                include G5_SHOP_PATH . '/payup/payup_cancel.php';
                break;
            default:
...
$is_member && $od_receipt_point && insert_point($member['mb_id'], (-1) * $od_receipt_point, "주문번호 $od_id 결제",'@payout', $od_id);
...
if ($config['cf_sms_use'] && ($default['de_sms_use2'] || $default['de_sms_use3'])) {
    ...
    if ($is_sms_send) {
        /**
         * @changelog 2022.06.21 Lee, Namdu
         * 문자 메시지 발송을 CoolSMS로 변경
         */
        !function_exists('sms_multi_send') && G5_EXTEND_PATH . '/mobile_message.extend.php';

        $entries = [];
        $queue = [];
        $order_amount = $tot_ct_price + $od_send_cost + $od_send_cost2;
        // 무통장 입금 때 고객에게 계좌정보 보냄
        $bank_info_message = "[{$config['cf_title']}] {$od_id} 주문 입금안내\n". str_replace('은행', '', $od_bank_account).' '.number_format($od_misu).'원';

        $default['de_sms_use2'] && ($entries[] = ['mobileNo' => $od_hp, 'text' => $od_settle_case == '무통장' ? $bank_info_message : $default['de_sms_cont2']]);

         // print strlen('Lee, Namdu님의 주문(2022071516053082) 입금안내: 신한 100-034-688436 18,000원');

        // 관리자에게 문자보내기
        $default['de_sms_use3'] && ($entries[] = ['mobileNo' => $default['de_sms_hp'], 'text' => $default['de_sms_cont3']]);
        foreach ($entries as $entry) {
            $queue[] = [
                preg_replace('/[^\d]+/', '', $entry['mobileNo']),
                str_replace([
                    '{이름}'
                    , '{보낸분}'
                    , '{받는분}'
        ...
        empty($queue) !== true && sms_multi_send($queue);
    }
}
// SMS END   --------------------------------------------------------
