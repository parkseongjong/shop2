...
$queue = [];
// 입금 확인 문자
if ($od_sms_ipgum_check && $default['de_sms_use4'] && $receive_number && $od_receipt_price && $od_deposit_name) {
    $queue[] = [
        preg_replace('/[^\d]+/', '', $receive_number), str_replace([
            '{이름}'
            , '{입금액}'
            , '{주문번호}'
            , '{회사명}'
            ...
            $od_name
            , number_format($od_receipt_price)
            , $od_id
            , $default['de_admin_company_name']
            , $config['cf_title']
        ], $default['de_sms_cont4'])
    ];

}
else if ($od_sms_baesong_check && $default['de_sms_use5'] && $receive_number && $od_delivery_company && $od_invoice) {
    $queue[] = [
        preg_replace('/[^\d]+/', '', $receive_number), str_replace([
            '{이름}'
            , '{택배회사}'
