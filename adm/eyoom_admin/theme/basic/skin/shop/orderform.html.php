	...
	<div id="anc_sodr_list">
                ...
                <table class="table table-bordered">
                    <thead>
                        <tr>
                            ...
                            <th style="text-align: left">
                                <label for="sit_select_all" class="checkbox" style="display: inline-block;margin:0;font-weight: 700">
                                    <input type="checkbox" id="sit_select_all" style="vertical-align: top"><i></i>
                                    주문번호 / 상품명
                                </label>
                    ...
                    <?php for ($i=0; $i<count((array)$list); $i++) { $opt = $list[$i]['opt'];?>
                        <?php for($k=0; $k<count((array)$opt); $k++) {
                            //
                            $po_point = $opt[$k]['ct_point'];
                            $agent_margin = $opt[$k]['ct_agent_margin'];

                            if($opt[$k]['mb_agent_code']) {
                                $po_point = Max($po_point - $agent_margin, 0);
                            }
                            else {
                                $agent_margin = 0;
                            }
                        ...
                        <tr>
                            ...
                            <td rowspan="<?php echo $list[$i]['rowspan']; ?>">
                                ...
                                <a href="<?php echo $list[$i]['href']; ?>" target="_blank" style="padding-left:27px"><strong><?php echo get_text($list[$i]['it_name']); ?></strong></a>
                            ...
                            <td class="text-right"><?php echo number_format($po_point); if($opt[$k]['mb_agent_code']) echo " <s class='text-warning'>(".number_format($opt[$k]['ct_point']).")</s>"?></td>
                            <td class="text-right"><?php echo number_format($agent_margin); ?></td>
        ...
        <div class="alert alert-warning">
            <p>
                ...
                개별적인(이곳에서의) 상태 변경은 모든 작업을 수동으로 처리합니다. 예를 들어 주문에서 입금으로 상태 변경시 입금액(결제금액)을 포함한 모든 정보는 수동 입력으로 처리하셔야 합니다.<br />
                <hr style="margin:.5rem 0;border-top: 1px solid #ECB45A" />
                <strong>"준비"</strong> 상태로 변경되면 대리점에 상품에 설정된 <strong>"영업 지급 포인트"</strong> 만큼 회원에게 지급될 포인트에서 차감되어 대리점에 지급됩니다.<br />
                <strong>"반품", "취소", "품절"</strong> 상태로 변경되면 대리점에 지급된 포인트가 차감됩니다.
					...
					<div class="row">
						...
						<div class="col col-8">
							<label class="checkbox" for="od_delivery_chk">
								<input type="checkbox" name="od_delivery_chk" id="od_delivery_chk" value="<?php echo $default['de_delivery_company'];  ?>" onclick="chk_delivery_company();"<?=!$od['od_delivery_company'] ? ' checked' : ''?>><i></i> 기본 배송회사로 설정
							</label>
							<label class="input" for="od_delivery_company">
								<input type="text" name="od_delivery_company" value="<?= $od['od_delivery_company'] ? $od['od_delivery_company'] : $default['de_delivery_company']?>" id="od_delivery_company">
							</label>
						</div>
					</div>
