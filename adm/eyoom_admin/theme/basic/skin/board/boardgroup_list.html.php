...
!function () {
    ...
    db.clients   = [
        <?php for ($i=0; $i<count((array)$list); $i++) { ?>
        {
            ...
            그룹관리자: "<?php if ($is_admin == 'super' && $is_admin == 'operator') { ?><label class='input'><input type='text' name='gr_admin[<?php echo $i; ?>]' id='gr_admin<?php echo $i; ?>' value='<?php echo get_sanitize_input($list[$i]['gr_admin']); ?>' style='text-align:right;'></label><?php } else { ?><input type='hidden' name='gr_admin[<?php echo $i ?>]' value='<?php echo get_sanitize_input($row['gr_admin']); ?>'><?php echo get_text($row['gr_admin']); ?><?php } ?>",
