        ...
        <div class="col col-9">
            <div class="padding-top-5">
                <span class="font-size-12 color-grey">
                    ...
                    <span>|</span>
                    <span style="color: red"> (소멸포인트합계 <?php echo number_format($expired); ?>점) </span>
                </span>
            </div>
        </div>
        <div class="col col-3">
...
!function () {
    ...
    db.clients   = [
        ...
        {
            'check': "<label for='chk_<?php echo $i; ?>' class='checkbox'><input type='checkbox' name='chk[]' id='chk_<?php echo $i; ?>' value='<?php echo $i; ?>'><i></i></label><input type='hidden' name='mb_id[<?php echo $i; ?>]' value='{.mb_id}' id='mb_id_<?php echo $i; ?>'><input type='hidden' name='po_id[<?php echo $i; ?>]' value='<?php echo $list[$i]['po_id']; ?>' id='po_id_<?php echo $i; ?>'>",
            'id': "<span class='ellipsis'><?php echo $list[$i]['mb_id']; ?></span>",
            'name': "<strong><?php echo $list[$i]['mb_name']; ?></strong>",
            'note': "<?php if ($list[$i]['link']) { ?><a href='<?php echo get_eyoom_pretty_url($list[$i]['po_rel_table'],$list[$i]['po_rel_id']); ?>' target='_blank'><span class='ellipsis'><?php echo $list[$i]['po_content']; ?></span></a><?php } else { ?><span class='ellipsis'><?php echo $list[$i]['po_content']; ?></span><?php } ?>",
            'point': "<?php echo number_format($list[$i]['po_point']); ?>",
            'datetime': "<?= (new DateTime($list[$i]['po_datetime']))->format('Y-m-d H:i') ?>",
            'expired': "<?php echo $list[$i]['expire_date']; ?>",
            'subtotal': "<?php echo number_format($list[$i]['po_mb_point']); ?>"
        },
        <?php } ?>
    ]
}();
...
$(function() {
    $("#point-list").jsGrid({
        ...
        height         : "auto",
        fields         : [
            { name: "check", 'title': '체크', type: "text", width: 40 },
            { name: "id", 'title': '아이디',type: "text", width: 100 },
            { name: "name",'title': '이름', type: "text", width: 120 },
            /*{ name: "닉네임", type: "text", width: 120 },*/
            { name: "note", 'title': '비고', type: "text", width: '' },
            { name: "point", 'title': '포인트', type: "number", width: 80 },
            { name: "datetime",'title': '지급일', type: "text", align: "center", width: 130 },
            { name: "expired", 'title': '만료일', type: "text", align: "center", width: 130 },
            { name: "subtotal", 'title': '소계', type: "number", width: 100 },
        ]
    });
