...
$sql = "SELECT COUNT(0) FROM {$g5['g5_shop_cart_table']} WHERE od_id = '{$od['cart_id']}' ";
if(!fn_sql_row($sql)) {
    alert("해당 주문번호로 주문 상품 정보가 존재하지 않습니다.");
}
...
echo 'document.location.href = "' . G5_ADMIN_URL . '/?dir=shop&pid=orderform&od_id='.$od_id.'";'.PHP_EOL;
echo 'else'.PHP_EOL;
echo 'document.location.href = "' . G5_ADMIN_URL . '/?dir=shop&pid=inorderlist&'.str_replace('&amp;', '&', $qstr).'";'.PHP_EOL;
echo '</script>'.PHP_EOL;