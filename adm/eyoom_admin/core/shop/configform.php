 ...
 */
// 페이업 결제 추가
if( ! isset($default['de_payup_mid']) ){
    $sql = "ALTER TABLE `{$g5['g5_shop_default_table']}` 
    ADD COLUMN `de_payup_mid` VARCHAR(255) NOT NULL DEFAULT '' AFTER `de_naverpay_sendcost`,
    ADD COLUMN `de_payup_secret` VARCHAR(255) NOT NULL DEFAULT '' AFTER `de_payup_mid`,
    ADD COLUMN `de_payup_apikey` VARCHAR(255) NOT NULL DEFAULT '' AFTER `de_naverpay_sendcost`;
            ";
    sql_query($sql, true);
}
