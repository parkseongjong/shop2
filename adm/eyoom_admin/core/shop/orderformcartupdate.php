...
function_exists('save_agent_margin') or include_once(G5_LIB_PATH . '/shop.lib.php');
...
$status_normal = ['주문', '입금', '준비', '배송', '완료'];
$status_cancel = ['취소', '반품', '품절'];
            ...
            if ($od['od_tno'] && ($od['od_settle_case'] == '신용카드' || $od['od_settle_case'] == '간편결제' || $od['od_settle_case'] == 'KAKAOPAY') || ($od['od_pg'] == 'inicis' && is_inicis_order_pay($od['od_settle_case']))) {
                switch ($od['od_pg']) {
                    ...
                    case 'KAKAOPAY':
                        ...
                        break;
                    case 'payup' :
                        $_POST['transactionId'] = $od['od_tno'];
                        $_POST['reason'] = '쇼핑몰 운영자 승인 취소';
...
// 미수금 등의 정보
$info = get_order_info($od_id, true);
if (!$info) {
...
else if (isset($_POST['ct_status']) && in_array($_POST['ct_status'], $status_normal)) { // 정상인 주문상태만 기록
...
if ($info['od_settle_case'] == '무통장' && $info['od_status'] == '주문' && $_POST['ct_status'] == '입금') {
    $od_sms_ipgum_check = true;
    $od_name = $info['od_name'];
    $od_hp = $info['od_hp'];
    $od_receipt_price = $info['od_cart_price'];
    $od_deposit_name = $info['od_deposit_name'];
    // SMS 문자전송
    define("_ORDERSMS_", true);
    include G5_ADMIN_PATH . "/shop_admin/ordersms.inc.php";
}
/**
 * @changelog 22.06.13 Lee, Namdu
 * 대리점 영업 수당 지급
 */
save_agent_margin($od_id);
