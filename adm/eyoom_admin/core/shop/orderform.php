...
for($i=0; $row=sql_fetch_array($result); $i++) {
     ...
     * 상품의 옵션정보
     */
    $sql = " select ct_id, it_id, ct_price, ct_point, ct_qty, ct_option, ct_status, cp_price, ct_stock_use, ct_point_use, ct_send_cost, io_type, io_price, mb_agent_code, ct_agent_margin
                from {$g5['g5_shop_cart_table']}
                where od_id = '{$od['od_id']}'
                  and it_id = '{$row['it_id']}'
