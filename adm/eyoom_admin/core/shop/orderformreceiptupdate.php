...
$Order = get_order_info($od_id, true);
$Order === false && alert('주문자료가 존재하지 않습니다.');

$posts['od_invoice'] = preg_replace('/[^\d]+/', '', $posts['od_invoice']);
...
if($Order['od_misu'] == 0 && $Order['od_status'] == '주문')
{
    $od_status = '입금';
    $cart_status = true;
}
...
if($posts['od_delivery_company'] && $posts['od_invoice'] && in_array($Order['od_status'], $order_status))
{
    $od_status = '배송';
    $cart_status = true;
    $od_sms_baesong_check = true;
}

// 미수금액
$od_misu = ( $Order['od_cart_price'] - $Order['od_cancel_price'] + (int) $posts['od_send_cost'] + (int) $posts['od_send_cost2'] )
           - ( $Order['od_cart_coupon'] + $Order['od_coupon'] + $Order['od_send_coupon'] )
           - ( (int) $posts['od_receipt_price'] + (int) $posts['od_receipt_point'] - (int) $posts['od_refund_price'] );

// 미수금 정보 등 반영
$sql = " update {$g5['g5_shop_order_table']}
            set 
                od_deposit_name    = '{$posts['od_deposit_name']}',
                od_bank_account    = '{$posts['od_bank_account']}',
                od_receipt_time    = '{$posts['od_receipt_time']}',
                od_receipt_price   = '{$posts['od_receipt_price']}',
                od_receipt_point   = '{$posts['od_receipt_point']}',
                ...
                od_misu         = '$od_misu',
                od_tax_mny      = '{$Order['od_tax_mny']}',
                od_vat_mny      = '{$Order['od_vat_mny']}',
                od_free_mny     = '{$Order['od_free_mny']}',
                od_status       = '$od_status'
            where od_id = '$od_id' ";
sql_query($sql);
...
// 배송때 재고반영
if($Order['od_misu'] == 0 && $od_status == '배송') {
...
if($_POST['od_tno'] && $_POST['od_escrow'] == 1)
{
    ...
    include(G5_SHOP_PATH.'/'.$Order['od_pg'].'/escrow.register.php');
}
