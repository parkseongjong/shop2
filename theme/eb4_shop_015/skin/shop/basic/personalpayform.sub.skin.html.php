                ...
                <div class="sod-frm-pay">
                    ...
                    <div class="personal-table">
                        <table>
                            ...
                            <tr>
                                <th scope="row"><label for="pp_name">이름<strong class="sound_only"> 필수</strong></label>
                                </th>
                                <td>
                                    <label for="pp_name" class="hidden-lg hidden-md">이름<strong class="sound_only">
                                            필수</strong></label>
                            ...
                            <tr>
                                <th scope="row"><label for="pp_email">이메일<strong class="sound_only"> 필수</strong></label>
                                </th>
                                <td>
                                    <label for="pp_email" class="hidden-lg hidden-md">이메일<strong class="sound_only">
                                            필수</strong></label>
                                    <label class="input width-200px required-mark">
                ...
                <div class="payment-select-wrap">
                    ...
                    <fieldset id="sod_frm_paysel">
                        ...
                        <?php if ($default['de_vbank_use']) {
                            ...
                            <input type="radio" id="pp_settle_vbank" name="pp_settle_case" value="가상계좌" <?php echo $checked; ?>>
                            <label for="pp_settle_vbank" class="payment-select-box vbank_icon"><?php echo $escrow_title; ?>
                                가상계좌</label>
                            <?php $checked = '';
                        } ?>
                        ...
                        <?php if ($default['de_iche_use']) {
                            ...
                            <input type="radio" id="pp_settle_iche" name="pp_settle_case" value="계좌이체" <?php echo $checked; ?>>
                            <label for="pp_settle_iche" class="payment-select-box iche_icon"><?php echo $escrow_title; ?>
                                계좌이체</label>
                            <?php $checked = '';
                        } ?>
                        ...
                        <?php if ($default['de_hp_use']) {
                            $multi_settle++; // 휴대폰 사용 ?>
                            <input type="radio" id="pp_settle_hp" name="pp_settle_case" value="휴대폰" <?php echo $checked; ?>>
                            <label for="pp_settle_hp" class="payment-select-box hp_icon">휴대폰</label>
                            <?php $checked = '';
                        } ?>

                        <?php if ($default['de_card_use']) {
                            $multi_settle++; // 신용카드 사용 ?>
                            <input type="radio" id="pp_settle_card" name="pp_settle_case" value="신용카드" <?php echo $checked; ?>>
                            <label for="pp_settle_card" class="payment-select-box card_icon">신용카드</label>
                            <?php $checked = '';
                        } ?>
...
<script type="text/javascript">
    function forderform_check(f) {
        ...
        for (var i = 0; i < settle_case.length; i++) {
            if (!settle_case[i].checked) continue;
            settle_check = true;
        ...
        else if (document.getElementById("pp_settle_iche") && document.getElementById("pp_settle_iche").checked && tot_price < 150) {
            alert("계좌이체는 150원 이상 결제가 가능합니다.");
            return false;
        }


        else if (document.getElementById("pp_settle_card") && document.getElementById("pp_settle_card").checked && tot_price < 1000) {
            alert("신용카드는 1000원 이상 결제가 가능합니다.");
        }

        else if (document.getElementById("pp_settle_hp") && document.getElementById("pp_settle_hp").checked && tot_price < 350) {
        ...
        <?php elseif($default['de_pg_service'] == 'lg') : ?>
        switch (settle_method) {
            case "계좌이체":
        ...
        <?php elseif($default['de_pg_service'] == 'inicis') : ?>
        switch (settle_method) {
            case "계좌이체":
        ...
        <?php elseif($default['de_pg_service'] == 'kcp') : ?>
        ...
        <?php elseif($default['de_pg_service'] == 'payup') : ?>
        $.ajax({
            'type': "POST",
            'data': $(f).serialize(),
            'url': g5_url + "/shop/ajax.orderdatasave.php",
        ...
        save_result ? alert(save_result) : checkoutPayup(f, settle_method);
        <?php endif;?>
    }
</script>