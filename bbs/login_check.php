...
in_array($mb['mb_id'], ['devper']) === true && ($_AGENT = 'DevelopmentTeam/HansBioTech');

set_session('ss_mb_key', md5($mb['mb_datetime'] . get_real_client_ip() . $_AGENT));
...
else if ($config['cf_cert_use'] && $config['cf_cert_hp'] && $mb['mb_certify'] != 'hp') {
    set_session('ss_nopasswd', 'login');
    alert('서비스 이용을 위해, 휴대전화 본인 확인이 필요합니다.\n\"회원 정보 수정\"에서 휴대전화 본인 인증을 하세요.', G5_BBS_URL . '/register_form.php');
}
