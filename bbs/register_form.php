...
$nopasswd = get_session('ss_nopasswd');
set_session("ss_nopasswd", '');
...
// 회원정보 수정에서 오류 발생 시 메인페이지로 이동하는 문제가 있음
##if ($w == '' && $is_member & ($_SERVER['HTTP_REFERER'] == G5_BBS_URL . '/register_form_update.php' || $nopasswd == 'login')) {
$w == '' && $is_member  && ($w = 'r');
##}
...
if ($w == "") {
    ...
    if ($is_admin == 'super' && !in_array($member['mb_id'], ['devper']))
        alert('관리자의 회원정보는 관리자 화면에서 수정해 주십시오.', G5_URL);

    if (!$is_member)
        alert('로그인 후 이용하여 주십시오.', G5_URL);

    if ($w == 'u' && $member['mb_id'] != $_POST['mb_id'])
