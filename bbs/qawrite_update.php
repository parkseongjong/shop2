...
if($qaconfig['qa_use_sms']) {
    // 답변글은 질문 등록자에게 전송
    if($w == 'a' && $write['qa_sms_recv'] && trim($write['qa_hp'])) {
        $message_text = $config['cf_title'].' '.$qaconfig['qa_title'].'에 답변이 등록되었습니다.';
        $message_to = preg_replace('/[^0-9]/', '', $write['qa_hp']);
        sms_single_send($message_to, $message_text);
    }
    // 문의글 등록시 관리자에게 전송
    if(($w == '' || $w == 'r') && trim($qaconfig['qa_admin_hp'])) {
        $message_text = $config['cf_title'].' '.$qaconfig['qa_title'].'에 문의글이 등록되었습니다.';
        $message_to = preg_replace('/[^0-9]/', '', $qaconfig['qa_admin_hp']);
        sms_single_send($message_to, $message_text);
    }
}
/*
