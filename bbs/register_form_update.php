...
if ($w == '' || $w == 'u') {
     ...
     */
    if ($config['cf_use_recommend'] && $mb_recommend && $recommendee == 'phone') {
    ...
    if ($w == '') {
        ...
        /**
         * @changelog 2022.07.26 by Lee, Namdu
         * - 닉네임 체크 제거(쇼핑몰인 관계로 의미 없음)
         */
...
if ($config['cf_cert_use'] && get_session('ss_cert_type') && get_session('ss_cert_dupinfo')) {
    $mb_dupinfo = fn_sql_quote(get_session('ss_cert_dupinfo'));
    $phone_no = fn_sql_quote($mb_hp);
    ...
    empty($exists) !== true && alert("입력한 개인정보로 가입된 내역(아이디: {$exists})이 존재합니다.");
}
...
if ($w == '') {
    ...
    // 추천인에게 포인트 부여
    if ($config['cf_use_recommend'] && $mb_recommend) {
        fn_reward_nominee($mb_recommend, $mb_id, $config['cf_recommend_point'], get_session('ss_cert_dupinfo'));
    }
    ...
    !trim(get_session('ss_mb_id')) && alert('로그인 되어 있지 않습니다.');

    (trim($_POST['mb_id']) != $mb_id) && alert("로그인된 정보와 수정하려는 정보가 틀리므로 수정할 수 없습니다.\\n만약 올바르지 않은 방법을 사용하신다면 바로 중지하여 주십시오.");

    $additional = '';
    empty($mb_password) !== true && ($additional .= ', mb_password =\'' . get_encrypt_string($mb_password) . '\'');
    ($mb_nick_default != $mb_nick) && ($additional .= ', mb_nick_date =\'' . G5_TIME_YMD . '\'');
    ($mb_open_default != $mb_open) && ($additional .= ', mb_open_date =\'' . G5_TIME_YMD . '\'');
    // 이전 메일주소와 수정한 메일주소가 틀리다면 인증을 다시 해야하므로 값을 삭제
    ($old_email != $mb_email && $config['cf_use_email_certify']) && ($additional .= ', mb_email_certify=\'\'');
    ($recommendee == 'phone' && $mb_recommend && $member['mb_recommend'] != $mb_recommend) && ($additional .= ', mb_recommend=\'' . $mb_recommend . '\'');
    ...
    /**
     * @changelog 2022.07.07 by Lee, Namdu
     * 대리점 코드가 있는 경우 '대리점' 테이블 데이터 갱신
     */
    if (empty($mb_1) !== true && $member['mb_1'] != $mb_1) {
        $sibling = fn_sql_row("SELECT COUNT(0) FROM {$g5['member_table']} WHERE mb_1='{$mb_1}'");
        sql_query("UPDATE {$g5['tb_agents']} SET ag_mentee='{$sibling}' WHERE ag_code='{$mb_1}'");
    }

    // 추천인에게 포인트 부여
    if ($config['cf_use_recommend'] && $mb_recommend && $member['mb_recommend'] != $mb_recommend) {
        fn_reward_nominee($mb_recommend, $mb_id, $config['cf_recommend_point'], $member['mb_dupinfo']);
    }
...
if ($w == '') {
    ...
    else if (empty($mb_password) !== true) {
        //$mb_password_re =
        set_session('ss_mb_id', '');
        alert('회원 정보가 수정 되었습니다.\n\비밀번호가 변경되었으므로 다시 로그인하셔야 합니다.', G5_URL);
    }
    else {
        echo '
        <!doctype html>
