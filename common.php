...
/*
 | PHP.ini 파일에서 세션이름 및 핸들러를 변경한다.
 | Redis 세션으로 변경.
 */
##-- @ini_set("session.save_handler", "files");
##-- session_save_path(G5_SESSION_PATH);
ini_set('session.save_handler', 'redis');
###session_save_path("unix:///var/run/redis/master.sock?database=".(G5_ENV == 'development' ? 4: 1));
session_save_path("tcp://" . G5_MYSQL_HOST . "?auth=" . G5_MYSQL_PASSWORD . "&database=" . (G5_ENV == 'development' ? 4 : 1));

defined('G5_SESSION_NAME') or define('G5_SESSION_NAME', 'BARRY2SID');
session_name(G5_SESSION_NAME);

// 모바일인 경우 15일, PC인 경우 4시간
$session_keep_alive = 60 * 60 * (G5_IS_MOBILE ? 15 * 24 : 4);
ini_set("session.cache_expire", $session_keep_alive); // 세션 캐쉬 보관시간 (분)
ini_set("session.gc_maxlifetime", $session_keep_alive); // session data의 garbage collection 존재 기간을 지정 (초)
...
session_set_cookie_params(G5_IS_MOBILE ? $session_keep_alive : 0, '/', G5_COOKIE_DOMAIN);

isset($SESSION_CACHE_LIMITER) !== true && ($SESSION_CACHE_LIMITER = 'no-cache, must-revalidate');
...
if( !function_exists('shop_check_is_pay_page') ){
    function shop_check_is_pay_page(){
        ...
        $pg_checks_pages = array(
            ...
            // 페이업
            $shop_dir.'/payup/payup_authorize.php',
            $shop_dir.'/payup/payup_checkout.php',
            $shop_dir.'/payup/payup_notify.php',
            $shop_dir.'/payup/payup_payout.php',
            $shop_dir.'/payup/payup_result.php',
            $shop_dir.'/payup/payup_vbank_transfer.php',
        );
...
// 4.00.03 : [보안관련] 세션ID 가 틀리면 로그아웃한다.
if (isset($_REQUEST[G5_SESSION_NAME]) && $_REQUEST[G5_SESSION_NAME] != session_id())
